      ...
      {links.map((link) => (
        <div key={link.name}>
          <div className="px-2 text-left md:cursor-pointer group gap-5">
            ...
            {link.submenu && (
              <div>
                {/* hidden chinh sua o day  */}
                <div className="absolute top-20 hidden  group-hover:md:block hover:md:block w-full left-0 mt-0 z-40 ">
                  {/* dropdown */}
                  <div className="bg-white p-7 grid grid-cols-4 gap-10 mt-2">
                    {link.sublinks.map((mysublinks, index) => (
                      <div key={`${mysublinks.Head}-${index}`}>
                        ...
                        <div className="flex flex-row">
                            <div className="grid grid-rows-3 gap-0 pt-2 ">
                            {mysublinks.sublink.map((slink, subIndex) => (
                              <li
                                key={`${slink.link}-${subIndex}`}
                                className="text-md text-gray-600 my-2.5 flex gap-2 hover:text-primary mr-15"
                              >
                                <div className="">
                                  <div className="flex justify-center items-center gap-0">
                                    <a
                                      className="justify-center items-center flex pr-2"
                                      style={{ fontSize: "2em" }}
                                    >
                                      {slink.icon}
                                    </a>
                                    ...
                                    <div className="flex flex-col">
                                      ...
                                      <Link
                                        to={slink.link}
                                        className="hover:text-primary font-serif"
                                      >
                                        {slink.name}
                                      </Link>
                                    </div>
                        ...
                        <div class="bg-slate-500 w-0.5 h-[300px] ml-9"></div>
                        </div>
                ...
                <div>
                  ...
                  >
                    {slinks.sublink.map((slink, subIndex) => (
                      <li
                        key={`${slink.link}-${subIndex}`}
                        className="py-3 pl-14 hover:text-primary"
                      >
